...
*	@details	This class is made to return Logmaster Sybsystem Control text and Hex value definition (int)**/
...
private:
	...
	bool fAllFieldsMessageFormatEnable; // Keep track of Message Format --all-off for correct togling of GUI check box.
	std::map < std::pair<int, int>, std::pair<eMSGLEVEL, eMSGSYSTEM> > fSybSysLevelXYTrack; // Keep track of X/Y position in GUI and mapping towards level and system 
	int fSystemsCount = 0;
	int fLevelsCount = 0;

	map<eMSGLEVEL, string> fLevels; 
	map<eMSGSYSTEM, string> fSystems;
