...
{
    // G_INFO("Sending message to channel %s",  message_info->fRequesChannel.c_str() );
...
{
    G_INFO("%s:  recieved blocking message to channel %s", GetName().c_str(), channel.c_str() );
    static thread_local std::mutex mtx;
    std::lock_guard<std::mutex> guard( mtx  );
...
{
    G_INFO("recived blocking message with timeout %d", timeout );
