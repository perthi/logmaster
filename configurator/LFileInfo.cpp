...
namespace CONFIGURATOR
{
        ...
        LFileInfo::ValidateClassName(const string classname)
    {
        ...
        {
            throw(std::invalid_argument(format("\"{}\". Is a directory", classname)));
        ...
        for ( auto& s : seps )
        {
            if ( g_string( )->Contains(classname, s) == true )
            {
                ...
                throw(std::invalid_argument(format("Invalid classname \"{}\". Must not contain suffix, or any special characters ", classname)));
        ...
        {
            throw(std::invalid_argument(format("Invalid classname \"{}\". Cannot start with a digit", classname)));
        }

        if ( g_number_types( )->IsAlphaNumber(classname) == false )
        {
            throw(std::invalid_argument(format("Invalid classname \"{}\". Must be a sequence of alphanumeric numbers (A-z, 0-9)", classname)));
        }
