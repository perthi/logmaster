...
/** @todo move or remove this class, it does not belong here */
class GComplexNumber
{
public:
	inline GComplexNumber()
	{
		fReal = 0.0;
		fImag = 0.0;
	}
	inline GComplexNumber(const GComplexNumber &o)
	{
		fReal = o.fReal;
		fImag = o.fImag;
	}
	inline GComplexNumber(float64_t preal, float64_t pimag)
	{
		fReal = preal;
		fImag = pimag;
	}
	...
	inline void SetData(float64_t preal, float64_t pimag)
	{
		fReal = preal;
		fImag = pimag;
	}
